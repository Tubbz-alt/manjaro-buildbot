...
git clone https://github.com/manjaro/manjaro-bootstrap.git
...
LOG_NAME=manjaro-bootstrap.log
LOG_LOC=/tmp/${LOG_NAME}

travis_ping start manjaro-bootstrap # since we are redirecting to log file
sudo ./manjaro-bootstrap.sh -d "${HOME}/pkg_download" "${CHROOT_DIR}" >> "${LOG_LOC}" 2>&1
travis_ping stop
